...
public class AILevel5 extends AIBase implements AIResponseInterface {

    private static final Logger logger = LoggerFactory.getLogger(AILevel5.class);


    private static final AILevel5 instance = new AILevel5();

    private AILevel5() {
    }

    public static AILevel5 getInstance() {
        return instance;
    }
...
//                        if (i < 27 && i % 9 <= 5) continue;//不能扔1-5点的牌
            ...
            int betterOut = discardCard(myResult, hands, remains, myWinDistance);
            if (betterOut != -1) {
                ret.setActionType(ACTION_DISCARD);
                ret.setCurTile(betterOut);
            }
    ...
    private void logicDiscardTile(ParamMode act) {
        if (act.getLastActionSeat() != act.getMySeat()
            ...
            } else {//没听牌，中等级打牌：能明杠返回明杠，碰牌后距离变小，就碰牌
                ackWhileOthersDiscardWhenNoTingMiddleLevel(hands,lastTile,ret);
            }
        }
    }
